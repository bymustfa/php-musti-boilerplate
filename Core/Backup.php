...
class Backup
{
    ...
    public function mysql($config = [])
    {
        ...
        foreach ($tables as $table) {
            ...
            // array_keys($tableDetail)[1]
            $oldkey = array_keys($tableDetail)[1];
            $newkey = str_replace(" ", "_", $oldkey);
            $tableDetail[$newkey] = $tableDetail[$oldkey];
            unset($tableDetail[$oldkey]);
            $this->sql .= $tableDetail[$newkey] . ';' . str_repeat(PHP_EOL, 3);
        ...
        return file_put_contents("compress.zlib://" . $this->config['db']['file'] . ".gz", $this->sql);
