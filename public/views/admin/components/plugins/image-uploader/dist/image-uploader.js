...
(function ($) {
  $.fn.imageUploader = function (options) {
    ...
    let defaults = {
      preloaded: [],
      imagesInputName: options?.name ? options.name : "images",
      preloadedInputName: "preloaded",
      label: options.label || "Sürükle ve bırak veya tıkla",
    ...
    let createContainer = function (options) {
      ...
      $input = $("<input>", {
        ...
        name: plugin.settings.imagesInputName + (options.multiple ? "[]" : ""),
    ...
    let createImg = function (src, id, preloaded) {
      ...
      if (options.showcase) {
        let $showcaseBtn = $("<button>", { class: "showcase-image" }).appendTo(
          $container
        );
        ...
        let $i = $("<i>", { class: "mdi mdi-radiobox-blank" }).appendTo(
          $showcaseBtn
        );
      }
    ...
    let validateExtension = function (file) {
      if (
        ...
        alert(
          `"${
            file.name
          }" bu dosya  kabul edilen dosya uzantılarıyla eşleşmiyor: "${plugin.settings.extensions.join(
            '", "'
          )}"`
        );
