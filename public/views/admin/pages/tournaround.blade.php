                ...
                <div class="page-title-box">
                    <div class="row align-items-center">
                        <div class="col-sm-12 d-flex align-items-center justify-content-between">
                            <h4 class="page-title">Yapılandırma ve Geçici Yöneticilik</h4>
                                ...
                                <div class="tab-pane p-3 border {{$language->id == "1"? "active" : ""}}"
                                    ...
                                    <p class="mb-0">
                                        <textarea lang-id="{{$language->id}}" lang-code="{{$language->short_code}}"
                                                  name="content">{{\App\Models\TournaroundPage::where(['lang_id' => $language->id])->get()->first()->content}}</textarea>
                                    </p>
                                </div>
                            @endforeach
...
@push('js')
    <script>
        ...
        $('#content_save').click(function () {
            ...
            $.post("{{base_url('api/pages/tournaround-update')}}", saveDatas, function (response) {
                if (response.status) {
