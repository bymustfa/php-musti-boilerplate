                                ...
                                <div class="tab-pane p-3 border {{$language->id == "1"? "active" : ""}}"
                                    ...
                                    <p class="mb-0">
                                        <textarea lang-id="{{$language->id}}" lang-code="{{$language->short_code}}"
                                                  name="content">{{\App\Models\HomeContent::where(['lang_id' => $language->id])->get()->first()->content}}</textarea>
                                    </p>
                                </div>
                            @endforeach
...
@push('js')
    <script>
        ...
        $('#content_save').click(function () {
            ...
            $.post("{{base_url('api/pages/home-update')}}", saveDatas, function (response) {
                if (response.status) {
